...
public class MovieRepositoryIT {
    ...
    @Inject
    private MovieRepository _movieRepository; // this is why we need an application server
...
//                .addAsLibraries(pomFile.resolve("org.hibernate:hibernate-core-jakarta:5.6.5.Final").withTransitivity().asFile())
    ...
    void shouldFindAll() {
        ...
        assertEquals(4, queryResultList.size()); // generally not the best to run a test on specific data. it's okay here; but really shouldn't hard code the value
