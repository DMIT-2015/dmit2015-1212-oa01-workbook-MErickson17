...
public class MovieRepository extends AbstractJpaRepository<Movie, Long> {
    ...
    public void create(Movie newMovie) {
        ...
        super.create(newMovie); // super means we access the abstract jpa create method
    }

    public List<Movie> list() {
        if (_securityContext.getCallerPrincipal() == null) {
            return super.list(); // is no one is logged in, we show all movies
        } else {
            String username = _securityContext.getCallerPrincipal().getName(); // logged in user - only show that user's username
            return getEntityManager().createQuery(
                            "FROM Movie m WHERE m.username = :usernameValue "
                            , Movie.class)
